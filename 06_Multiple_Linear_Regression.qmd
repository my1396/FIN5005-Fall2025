...
<!-- The code above is only displayed (eval=FALSE); the chunk below re-runs
     it via ref.label and prints the table. For PDF the table sits on a
     landscape page and is scaled with adjustbox. The raw-LaTeX wrappers are
     kept as literal {=latex} blocks (not cat() from R) so Quarto passes them
     through reliably; only the table -- not the echoed source code -- goes
     inside adjustbox, otherwise the verbatim listing breaks the box. In HTML
     these blocks are dropped and a normal table is shown. -->
