...
where STR is the student-teacher ratio (class size).
...
```{r #fig-scatter-1, fig.asp=0.4, fig.cap="Scatter plots of TestScore vs expenditure and STR", fig.width=12, echo=FALSE}
long_format <- cas %>%
    select(TestScore, expenditure, STR) %>%
    pivot_longer(
...
- We can therefore consider `calworks` as a **superfluous control variable**, given the inclusion of `lunch` in this model. 
  ...
  ```{r #fig-scatter-2, fig.cap="Scatter plots of lunch vs calworks", echo=FALSE}
