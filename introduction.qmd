...
- Compute and explain expectation, variance (including shortcut $E[X^2]-(E[X])^2$), and standard deviation in business contexts.
- Interpret higher moments: skewness ($\mu_3$) and kurtosis ($\mu_4$) for tail and asymmetry assessment.
...
Probability is the foundation for making data-driven decisions in business. This section covers the essential concepts, formulas, and advanced business examples you’ll use in analytics.
...
<div id="sol-1" class="solution-answer env-green">$0.8$</div>
...
**Advanced Example:**
A telecom company surveys 500 customers about service quality. The sample mean satisfaction score is 4.2. How can this estimate help guide company-wide improvements?
...
<div id="sol-10" class="solution-answer env-green">$\mathbb{E}[X] = 0(0.2)+1(0.5)+2(0.3)=1.1$. $\mathbb{E}[X^2] = 0^2(0.2)+1^2(0.5)+2^2(0.3)=0+0.5+1.2=1.7$.  
Variance $=1.7 - 1.1^2 = 1.7 - 1.21 = 0.49$.</div>
...
<div id="sol-11" class="solution-answer env-green">Mean $= (0.4+0.6-0.2+0.5+0.7)/5 = 0.4$. $\mathbb{E}[X^2] = (0.16+0.36+0.04+0.25+0.49)/5 = 1.30/5 = 0.26$.  
Variance $= 0.26 - 0.4^2 = 0.26 - 0.16 = 0.10$. Direct method gives the same (0.10). Interpretation: moderate short‑term volatility; product or marketing experiments causing swings should be monitored if variance rises.</div>
...
1. **Sum of squared deviations**  
  ...
  \end{aligned}
  $$
1. **Population variance** (divide by $n=5$)  
  $$ \sigma^2 = \frac{2680}{5} = 536 $$
1. **Sample variance** (unbiased, divide by $n-1=4$)  
  $$ s^2 = \frac{2680}{4} = 670 $$
...
<div id="sol-5" class="solution-answer env-green">Higher tail risk: higher chance of extreme profit or loss; focus risk management on outliers.</div>
...
4. Higher-moment estimators (skewness, kurtosis) include finite-sample correction factors to reduce bias.
5. Always interpret sample statistics as **estimates** with sampling variability; risk management and forecasting should account for estimation error (e.g., confidence intervals, simulation, bootstrapping).
...
Interpretation tip: If a sample statistic looks extreme (e.g., very high skewness or kurtosis), examine sample size and outliers; small samples amplify noise in higher-moment estimates.
