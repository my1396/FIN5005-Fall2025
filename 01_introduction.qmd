...
```{r, child="_chunk-opt.qmd", include=FALSE}
```
...
- **Variance (population):** How much values fluctuate around the true mean.
  ...
  **Sample estimators:**
  ...
  - **Unbiased** estimator: 
    ...
    The latter ($s^2$) subtracts 1 from $n$ in the denominator, which is known as a <span class="env-green">degrees of freedom correction</span>. 
    This version has some desirable properties but we will not discuss these for now. Suffice to say that both versions are usually fine in practice.
...
knitr::include_graphics("images/t_distribution2.png")
```
...
include_gif("images/t-dist_chang_df.gif") 
```
